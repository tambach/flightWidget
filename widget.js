...
class Widget extends Try {
	...
	constructor(model, view, controller, app) {
		...
		this.try.name = this.constructor.name.toLowerCase();
	...
	*/
	store(field, value) {
		Store.set(this.try.name + "_" + field, value);
	}
	...
	*/
	restore(field) {
		return Store.get(this.try.name + "_" + field);
	}
	...
	*/
	has(field) {
		return Store.has(this.try.name + "_" + field);
	}
	...
	*/
	destroy(field) {
		Store.destroy(this.try.name + "_" + field);
	}
...
class WidgetView extends Try {
	...
	draw() {
		...
		this.try.stage = HTML.create("div");
		CSS.style(this.try.stage, {"position": "relative", "width": "150px", "height": "150px", "overflow": "hidden", "backgroundColor": "rgba(200, 200, 200, 1)", "borderRadius": this.try.mvc.main.radius});
		
		if(this.try.mvc.main.header) {
			this.try.header = HTML.create("div");
			CSS.style(this.try.header, {"z-index": 100, "userSelect": "none", "width": "100%", "height": "25px", "lineHeight": "25px", "textAlign": "center", "border-top-left-radius": this.try.mvc.main.radius, "border-top-right-radius": this.try.mvc.main.radius,"backgroundColor": "#0072ff"});
			this.try.header.innerHTML = this.try.mvc.main.name;
			this.try.stage.appendChild(this.try.header);
		}
		
		if(this.try.mvc.main.footer) {
			this.try.footer = HTML.create("div");
			CSS.style(this.try.footer, {"position": "absolute", "bottom": "0px", "z-index": 100, "width": "100%", "height": "25px", "lineHeight": "25px", "textAlign": "center", "border-bottom-left-radius": this.try.mvc.main.radius, "border-bottom-right-radius": this.try.mvc.main.radius,"backgroundColor": "#0072ff"});
